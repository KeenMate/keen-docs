...
var AppsManager = class {
  ...
  async create(name, target, opts) {
    ...
    if (typeof mount !== "function") {
      throw new Error(
        `[keen_phoenix_svelte] app "${name}" must have a default export (target, { props, context, live, api, channel, bus, el }) => handle`
      );
    }
...
function getChannelFactory(context) {
  return (topic, params = {}) => {
    const socket = getSocket(context);
    const channel = socket.channel(topic, params);
    const joined = new Promise((resolve, reject) => {
      channel.join().receive("ok", resolve).receive("error", reject).receive("timeout", () => reject(new Error("join timeout")));
    });
    return {
      channel,
      // raw Phoenix channel for full control
      joined,
      push(event, payload = {}, opts = {}) {
        const body = opts.cid === false ? payload : { ...payload, cid: opts.cid || genCid() };
        return new Promise((resolve, reject) => {
          channel.push(event, body).receive("ok", resolve).receive("error", reject).receive("timeout", () => reject(new Error("push timeout")));
        });
      },
...
// rt/bus.js
function createBus(target = new EventTarget()) {
...
function getAppsManifest() {
  if (_appsManifest === void 0) {
...
function mountStatic(appsManager2, root = document) {
  root.querySelectorAll("[data-app]").forEach((el) => {
    ...
    if (inLiveView && !eager) return;
    el.__keenMounted = true;
    if (inLiveView && eager) el.__keenEager = true;
    el.__keenReady = appsManager2.create(el.dataset.app, el, {
      props: parseProps(el),
      context: getContext(),
      live: null,
      ...
      bus: getBus(),
      el
    }).then((instance) => el.__keenInstance = instance).catch((err) => {
      console.error(err);
      return null;
    });
  });
...
// rt/index.js
var appsManager = new AppsManager({ manifest: getAppsManifest });
function mountStatic2(root) {
  return mountStatic(appsManager, root);
}
var KeenApp = {
  mounted() {
    ...
    this.el.__keenMounted = true;
    appsManager.create(name, this.el, {
...
function buildLive(hook) {
  return {
    ...
    pushEventTo: (target, event, payload, onReply) => hook.pushEventTo(target || hook.el, event, payload, onReply),
...
function getHooks(extra = {}) {
  return { KeenApp, ...extra };
}
var index_default = getHooks;
export {
  AppsManager,
  KeenApp,
  appsManager,
