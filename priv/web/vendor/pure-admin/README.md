...
`pa-table`/`pa-profile-panel`/…), utilities, dark mode (`.pc-mode-dark`), and a **baked-in default
...
modes (`pc-mode-*`) and variants (`pa-color-*`).
...
`navbar-collapse.js`, and `sidebar-resize.js`). Grid columns (`.pc-col-*`) live in pure-css
