defmodule KeenDocs.Markup do
  ...
  **layout** slots (Level-2 structure) so content columns render on `@keenmate/pure-css`'s native
  ...
  when the ratio reduces to one pa-grid ships (`80/20` → `.pc-col-4-5`/`.pc-col-1-5`, thirds →
  `.pc-col-1-3`), otherwise to the nearest 5% column (`.pc-col-75`). A standalone `:::col` (no
  width) is an auto/equal `.pc-col`.
  """
  ...
  defp columns(a, _cls), do: [~s(<div class="pc-row">), a.body, "</div>"]

  # Non-column children span the full row.
  defp span(a, _cls), do: [~s(<div class="pc-col-100">), a.body, "</div>"]

  # :::col -> a sized pc-col carrying the (optionally accented) label + body. The label/body keep
  # the engine's km-col__* classes (styled in keendocs.css); only the wrapper becomes a pc-col.
  defp col(a, cls) do
    [
  ...
  # {part, total} -> a pc-col class: an exact fraction when the reduced ratio is one pa-grid ships,
  # else the nearest 5% column. nil -> auto/equal column.
  defp pa_col(nil), do: "pc-col"
