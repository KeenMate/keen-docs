defmodule KeenDocs.Web.View do
  ...
  def layout(title, inner, opts \\ []) do
    ...
    aside =
      if Map.get(regions, "sidebar", true) != false do
        # Always render a sidebar host — even sidebar-less pages need the navbar-collapse OVERFLOW
    ...
    <body class="pa-layout--sticky">
      <nav class="pa-navbar">
        <div class="pa-navbar__inner">
          <div class="pa-header__start">
            <button class="pa-header__burger burger-menu" type="button" onclick="kdToggleSidebar()" aria-label="Toggle sidebar"><span></span><span></span><span></span></button>
            <div class="pa-header__brand">#{brand_html(rb, docset)}</div>
      ...
      <div class="pa-layout#{layout_mod}">
        <div class="pa-layout__inner">
          #{aside}<div class="pa-layout__content">
            <main class="pa-layout__main">
              <div class="pa-layout__main__inner#{inner_mod}">
    #{page_head}#{content}
              </div>
            </main>
          </div>
    ...
    #{footer}  </div>
      ...
      <script src="/vendor/pure-admin/navbar-collapse.js"></script>
      <script src="/vendor/pure-admin/sidebar-resize.js"></script>
  ...
  defp search_form(opts, regions) do
    if Map.get(regions, "search", "center") == "off" do
      ...
      # pure-admin's `.pa-navbar-search` is a click-trigger box (icon · placeholder · kbd), not an
      # input. We keep the native chrome but slot a real submittable <input> where the placeholder
      # span sits — the first place the pure-admin contract had to bend (see harness `.pa-navbar-search__input`).
      ~s(<form class="pa-navbar-search pa-navbar-search--md kd-search" action="/search" method="get" role="search">) <>
        ~s(<span class="pa-navbar-search__icon" aria-hidden="true">&#128269;</span>) <>
        ~s(<input class="pa-navbar-search__input" name="q" value="#{esc(opts[:q] || "")}" placeholder="Search docs…" autocomplete="off" aria-label="Search docs" />) <>
        ~s(<span class="pa-navbar-search__shortcut" aria-hidden="true"><kbd>/</kbd></span>) <>
        ~s(</form>)
    end
  end
  ...
  # The pa-header__end cluster, composed per the header block: per-doc_set links, the Resolve
  ...
  # both flex-shrink:0 in pure-admin's header).
  ...
  # pure-admin's own `.pa-btn` so it inherits the active theme's button skin. `icon` is emitted
  ...
  defp header_cta_html(%{"url" => url} = cta) when is_binary(url) do
    ...
    icon = if cta["icon"], do: ~s(<span class="pa-btn__icon" aria-hidden="true">#{cta["icon"]}</span>), else: ""
    label = if cta["label"], do: ~s(<span class="kd-cta__label">#{esc(cta["label"])}</span>), else: ""
    ~s(<a class="pa-btn pa-btn--#{esc(variant)} pa-btn--sm kd-cta" href="#{esc(url)}" target="_blank" rel="noopener">#{icon}#{label}</a>)
  end
  ...
  # A named layout variant → a `kd-layout--<slug>` hook class on `.pa-layout` ("default"/absent → none).
  ...
  # Progressive-enhancement JS mirroring pure-admin's navbar: the burger toggles a
  ...
  # Profile panel behaviour (mirrors pure-admin): the header button toggles `.pa-profile-panel--open`
  ...
  defp profile_js do
    "function kdToggleProfile(){var p=document.getElementById('profilePanel');if(!p)return;" <>
      "var o=p.classList.toggle('pa-profile-panel--open');document.body.classList.toggle('pa-scroll-lock',o);}" <>
      "function kdCloseProfile(){var p=document.getElementById('profilePanel');if(!p)return;" <>
      "p.classList.remove('pa-profile-panel--open');document.body.classList.remove('pa-scroll-lock');}" <>
      "document.addEventListener('click',function(e){var p=document.getElementById('profilePanel')," <>
      "b=document.querySelector('.pa-header__profile-btn');if(p&&b&&p.classList.contains('pa-profile-panel--open')" <>
      ...
      "document.querySelectorAll('[data-profile-tab]').forEach(function(x){x.classList.toggle('pa-tabs__item--active',x.dataset.profileTab===id);});" <>
      "document.querySelectorAll('[data-profile-panel]').forEach(function(x){x.classList.toggle('pa-tabs__panel--active',x.dataset.profilePanel===id);});" <>
      "});});" <>
      "document.querySelectorAll('.pa-profile-panel__favorite-item').forEach(function(i){i.addEventListener('click',function(e){" <>
      "if(e.target.closest('.pa-profile-panel__favorite-remove'))return;if(i.dataset.href)location.href=i.dataset.href;});});" <>
      "document.querySelectorAll('.pa-profile-panel__favorite-remove').forEach(function(b){b.addEventListener('click',function(e){" <>
  ...
  def sidebar_html(set, nav_rows, active_variant, active_slug, variants, docset \\ nil) do
    ...
    items =
      Enum.map_join(nav_rows, "", fn n ->
        ...
        if n.is_section do
          ~s(<li class="pa-sidebar__section"#{ind}>#{esc(n.label)}</li>)
        else
          v = n.variant_code || base_variant
          show = Map.get(vmap, v, true)
          active = if n.slug == active_slug, do: " pa-sidebar__link--active", else: ""

          ~s(<li class="pa-sidebar__item"#{ind}><a class="pa-sidebar__link#{active}" href="#{doc_path(set, v, n.slug, show)}"><span class="pa-sidebar__label">#{esc(n.label)}</span></a></li>)
        end
      end)

    nav = if items == "", do: "", else: ~s(<nav class="pa-sidebar__nav"><ul>#{items}</ul></nav>)

    # The navbar→sidebar overflow is now handled natively by pure-admin's navbar-collapse.js (it
      ...
      rows ->
        ...
        items =
          Enum.map_join(tops, "", fn n ->
            ...
            act = if active, do: " pa-header__nav-item--active", else: ""
            # rc11 no longer auto-pins the active item — pin it ourselves so the current section keeps
            # its bar spot and the rest collapse into the sidebar first (highest priority drops last).
            prio = if active, do: ~s( data-pa-nav-priority="100"), else: ""

            if n.is_section and kids != [] do
              {href, _ext?} = nav_href(n)
              menu = Enum.map_join(kids, "", fn k -> ~s(<li>#{nav_link(k)}</li>) end)

              ~s(<li class="pa-header__nav-item pa-header__nav-item--has-dropdown#{act}"#{prio}>) <>
                ~s(<a class="pa-header__nav-link" href="#{esc(href)}">#{esc(n.label)}</a>) <>
                ~s(<ul class="pa-header__dropdown">#{menu}</ul></li>)
            else
              ~s(<li class="pa-header__nav-item#{act}"#{prio}>#{nav_link(n)}</li>)
            end
          end)

        # pure-admin's navbar-collapse.js (rc09): as the header narrows it folds the lowest-priority
        # top-nav items into the sidebar (target #kd-nav-overflow) under a "Browse" .pa-sidebar__section
        # — leaves become links, dropdowns become collapsible groups. The --active item auto-pins.
        ~s(<nav class="pa-header__nav pa-header__nav--start" data-pa-nav-collapse="sidebar" data-pa-nav-collapse-label="Browse" data-pa-nav-collapse-target="#kd-nav-overflow" data-pa-nav-collapse-icon=""><ul>#{items}</ul></nav>)
    end
  end
  ...
  # A plain nav anchor — no class; pure-admin styles it via `.pa-header__nav > ul > li > a`
  # (top level) and `.pa-header__dropdown li a` (menu items), skinned by the active theme.
  defp nav_link(n) do
  ...
  # ── presentation template resolution ────────────────────────────────────────
  # The page stylesheets. A single self-contained framework bundle is linked first — the vendored
  # pure-admin `core.css` (10px rem base + reset + grid + utilities + all pa-* chrome + a baked
  ...
  defp styles(docset) do
    case theme_id(docset) do
      nil ->
        # Baseline: core.css (framework + baked light palette) + keen-docs' dark override + glue.
        core_link() <>
          ~s(  <style>#{dark_theme_css()}#{theme_css(docset)}#{harness_css()}#{content_css()}</style>)

      id ->
        if KeenDocs.Themes.overlay?(id) do
          # keen-docs-authored OVERLAY theme (base: core): layer the skin ON core.css. Order —
          # core (framework) → harness+content (keen-docs glue) → the theme skin (overrides both,
          # owns its own light/dark) → the per-doc_set accent/vars override (wins last).
          core_link() <>
            ~s(  <style>#{harness_css()}#{content_css()}</style>\n) <>
            theme_link(id) <>
            ~s(  <style>#{theme_css(docset)}</style>)
        else
          # Self-contained pure-admin bundle: it carries its own framework + light/dark, so it
          # REPLACES core.css. keen-docs still inlines the accent/vars override + its kd-* content.
          theme_link(id) <>
            ~s(  <style>#{theme_css(docset)}#{harness_css()}#{content_css()}</style>)
        end
    end
  end

  defp core_link, do: ~s(  <link rel="stylesheet" href="/vendor/pure-admin/core.css" />\n)
  defp theme_link(id), do: ~s(  <link rel="stylesheet" href="/themes/#{id}/dist/#{id}.css" />\n)

  # Which installed theme bundle presents this doc_set. One global default
  # (`config :keen_docs, :theme`), overridable per doc_set via `settings["theme"]["id"]` — mirrors
  # the keen_markdown profile resolution. The same `settings["theme"]` map also carries the
  # accent/vars micro-override (read by theme_css/1); `id` selects the base bundle it tweaks.
  # "none"/nil/absent → the vendored core.css baseline. Guarded to an id whose bundle is installed.
  defp theme_id(docset) do
    case get_in(settings(docset), ["theme", "id"]) || Application.get_env(:keen_docs, :theme) do
      id when is_binary(id) and id != "none" ->
        if KeenDocs.Themes.installed?(id), do: id, else: nil
  ...
  # The pa-color-* class for the active theme's default colour variant (or "" — core baseline and
  # single-variant themes like nato/dracula/corporate). Theme-defined via the bundle's theme.json.
  defp theme_variant_class(docset) do
    case theme_id(docset) do
      nil -> ""
      id -> KeenDocs.Themes.default_variant_class(id)
    end
  end
  ...
  # The navbar trigger for the profile panel (pure-admin's `pa-header__profile-btn`).
  defp profile_btn_html do
    ...
    ~s(<button type="button" class="pa-header__profile-btn" onclick="#{onclick}" aria-label="Open profile"><span class="pa-btn__icon">&#128100;</span><span class="pa-header__profile-name">Guest</span></button>)
  end

  # The slide-in profile panel — same anatomy as pure-admin (header · tabs · Profile nav · Favorites ·
  ...
  defp profile_panel_html do
    ...
    <div class="pa-profile-panel" id="profilePanel">
      <div class="pa-profile-panel__overlay" onclick="kdCloseProfile()"></div>
      <div class="pa-profile-panel__content">
        <div class="pa-profile-panel__header">
          ...
          <div class="pa-profile-panel__info">
            <h3 class="pa-profile-panel__name" title="Guest">Guest</h3>
            <p class="pa-profile-panel__email" title="not signed in">not signed in</p>
            <span class="pa-badge">Reader</span>
          </div>
          <button class="pa-profile-panel__close" onclick="kdCloseProfile()" aria-label="Close profile">&#10005;</button>
        </div>
        <div class="pa-profile-panel__tabs">
          <div class="pa-tabs pa-tabs--full">
            <button class="pa-tabs__item pa-tabs__item--active" data-profile-tab="profile" title="Profile"><span class="pa-profile-panel__tab-text">Profile</span></button>
            <button class="pa-tabs__item" data-profile-tab="favorites" title="Favorites"><span class="pa-profile-panel__tab-text">Favorites</span></button>
          </div>
        </div>
        <div class="pa-profile-panel__body">
          <div class="pa-tabs__panel pa-tabs__panel--active" data-profile-panel="profile">
            <nav class="pa-profile-panel__nav"><ul>
              <li><a href="#" class="pa-profile-panel__nav-item"><span class="pa-profile-panel__nav-icon">&#128100;</span>Profile settings</a></li>
              <li><a href="#" class="pa-profile-panel__nav-item"><span class="pa-profile-panel__nav-icon">&#128221;</span>My notes</a></li>
              <li><a href="#" class="pa-profile-panel__nav-item"><span class="pa-profile-panel__nav-icon">&#9881;</span>Preferences</a></li>
              <li><a href="#" class="pa-profile-panel__nav-item"><span class="pa-profile-panel__nav-icon">&#10067;</span>Help &amp; support</a></li>
            </ul></nav>
          </div>
          <div class="pa-tabs__panel" data-profile-panel="favorites">
            <div class="pa-profile-panel__favorites"><ul>
              <li><div class="pa-profile-panel__favorite-item" data-href="/web-multiselect"><span class="pa-profile-panel__favorite-icon">&#11088;</span><span class="pa-profile-panel__favorite-label">Web MultiSelect</span><button class="pa-profile-panel__favorite-remove" title="Remove">&#10005;</button></div></li>
              <li><div class="pa-profile-panel__favorite-item" data-href="/backend-guidelines"><span class="pa-profile-panel__favorite-icon">&#11088;</span><span class="pa-profile-panel__favorite-label">Backend guidelines</span><button class="pa-profile-panel__favorite-remove" title="Remove">&#10005;</button></div></li>
            </ul></div>
            <div class="pa-profile-panel__favorites-add"><button class="pa-btn pa-btn--sm pa-btn--outline-secondary pa-btn--block">+ Add current page</button></div>
          </div>
        </div>
        <div class="pa-profile-panel__footer">
          <button class="pa-btn pa-btn--secondary pa-btn--block">Sign in</button>
  ...
  # The floating reader settings panel — pure-admin's `.pa-settings-panel` offcanvas (a gear tab on
  # the page edge, styled entirely by the vendored core.css). Its control set is scoped to what
  ...
  defp settings_panel_html do
    """
    <div class="pa-settings-panel" id="kdSettingsPanel">
      <button class="pa-settings-panel__toggle" id="kdSettingsToggle" type="button" title="Settings" aria-label="Settings">&#9881;</button>
      <div class="pa-settings-panel__content">
        <h3 class="pa-settings-panel__title">Settings</h3>
        <div class="pa-settings-panel__section">
          <label class="pa-settings-panel__label" for="kdModeSelector">Appearance</label>
          <select class="pa-settings-panel__select" id="kdModeSelector">
        ...
        <div class="pa-settings-panel__section">
          <label class="pa-settings-panel__label" for="kdFontSizeSelector">Font size</label>
          <select class="pa-settings-panel__select" id="kdFontSizeSelector">
            <option value="small">Small</option>
            <option value="default">Default</option>
            <option value="large">Large</option>
            <option value="xlarge">Extra large</option>
          </select>
          <small class="pa-settings-panel__hint">Rescales the page. All elements scale proportionally.</small>
        </div>
        <div class="pa-settings-panel__section">
          <label class="pa-settings-panel__label" for="kdFontFamilySelector">Font family</label>
          <select class="pa-settings-panel__select" id="kdFontFamilySelector">
        ...
        <div class="pa-settings-panel__section">
          <label class="pa-settings-panel__label" for="kdSidebarBehaviorSelector">Sidebar</label>
          <select class="pa-settings-panel__select" id="kdSidebarBehaviorSelector">
          ...
          <div class="pa-settings-panel__checkbox-group">
            <label class="pa-checkbox">
              ...
              <span class="pa-checkbox__label">Resizable (drag the edge)</span>
            </label>
          </div>
        </div>
        <div class="pa-settings-panel__section">
          <button class="pa-btn pa-btn--secondary pa-btn--block" id="kdResetSettings" type="button">Reset to defaults</button>
        </div>
      </div>
    </div>
    """
  ...
  # The page footer — pure-admin's `pa-layout__footer` anatomy (start / center / end sections).
  ...
  defp docset_footer(docset) do
    ...
    ~s(<footer class="pa-layout__footer">) <>
      ~s(<div class="pa-footer__start"><p class="m-0">#{left}</p></div>) <>
      ~s(<div class="pa-footer__center">#{links}</div>) <>
      ~s(<div class="pa-footer__end pa-footer__end--vertical">#{social}</div>) <>
      ~s(</footer>)
  end
  ...
  defp harness_css do
    ...
    /* dropdown caret drawn as a pseudo-element (NOT text) so navbar-collapse.js's labelOf() doesn't
    ...
    .pa-header__nav > ul > li.pa-header__nav-item--has-dropdown > a::after{content:"▾";font-size:.85em;opacity:.6;margin-inline-start:.4rem}
    /* active top-nav item + the navbar-collapse "Browse" section come from core (rc09):
    ...
    /* Reserve the search's min-width on the CENTER FLEX ITEM, not just the input. Flexbox only honours
       ...
       forces flex to shrink `start` to make room, which shrinks the nav's slot — so navbar-collapse.js
    ...
    /* The stacked brand wordmark is ~190px wide and won't shrink (nowrap); below tablet it stops the
       navbar-collapse from freeing enough room (start can't shrink past brand+items, so it overflows
    ...
    .pa-layout__main__inner--wide{max-width:134.4rem}
    ...
    .kd-sidebar:not(:has(.pa-sidebar__item)):not(:has(.pa-sidebar__section)){display:none}
    /* ---- footer link colours (structure/look from core.css) ---- */
    .pa-layout__footer a{color:var(--base-accent-color,#2563eb)} .kd-social{margin-inline-end:0.96rem}
    /* ---- rendered content ---- */
